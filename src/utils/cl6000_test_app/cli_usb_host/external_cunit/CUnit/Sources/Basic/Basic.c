                                                ...
                                                const CU_pFailureRecord pFailureList)
{
  ...
  if (CU_BRM_SILENT == f_run_mode) {
    fprintf(stdout, "%s: %s::%s\n",
            pFailureList ? "FAIL" : "PASS",
            pSuite->pName ? pSuite->pName : "(null)",
            pTest->pName ? pTest->pName : "(null)");
    return;
  }
  ...
  if (NULL == pFailure) {
    if (CU_BRM_VERBOSE == f_run_mode) {
      fprintf(stdout, "\nPASS: %s::%s", (NULL != pSuite->pName) ? pSuite->pName : "",
    ...
    switch (f_run_mode) {
      case CU_BRM_VERBOSE:
        fprintf(stdout, "\nFAIL: %s::%s", (NULL != pSuite->pName) ? pSuite->pName : "",
                                         (NULL != pTest->pName) ? pTest->pName : "");
        break;
      case CU_BRM_NORMAL:
