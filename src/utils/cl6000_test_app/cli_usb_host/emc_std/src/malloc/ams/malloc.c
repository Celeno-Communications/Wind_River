/**
 * @file
 * @brief   Malloc emulation based on AMS memory allocation functions.
...
{
  int i;
  fprintf(stderr, "***\n*** emc_heap_print(\"%s\"): current=%u max=%u total=%u\n",
          who ? who : "", emc_heap_current, emc_heap_max, emc_heap_total);
  fprintf(stderr, "***");
  for (i = 0; i < sizeof(emc_heap_frees) / sizeof(emc_heap_frees[0]); ++i) {
    fprintf(stderr, " %u", emc_heap_mallocs[i] - emc_heap_frees[i]);
  }
  fprintf(stderr, "\n***");
  for (i = 0; i < sizeof(emc_heap_frees) / sizeof(emc_heap_frees[0]); ++i) {
    fprintf(stderr, " %u-%u", emc_heap_mallocs[i], emc_heap_frees[i]);
  }
  fprintf(stderr, "\n***\n");
}
...
void* emc_std_malloc(size_t size)
{
  ...
  if (!size) {
    ...
    // if (1 == size) { fprintf(stderr, "emc_ams_malloc(1): %p\n", ptr); }
    // *ptr = size;
    // ptr += 1;
    emc_heap_total += size;
  ...
  if ((void*)-1 != pointer) {
    ...
    if (pointer) {
      ...
      *ptr = *(((unsigned int*)(ptr)) - 1) - 4;
      ...
      // if (1 == *ptr) { fprintf(stderr, "emc_ams_free(1): %p\n", ptr); }
    }
#endif /* EMC_MALLOC_COUNT */
