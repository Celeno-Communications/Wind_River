...
static int ce_last_scan_res_compare(const void *a, const void *b)
{
	struct wpa_bss * const *_bss_a = a;
	struct wpa_bss * const *_bss_b = b;
	struct wpa_bss *bss_a = *_bss_a;
	struct wpa_bss *bss_b = *_bss_b;

	if (bss_a->level > bss_b->level)
		return -1;
	else if (bss_a->level < bss_b->level)
		return 1;
	else
		return 0;
}

static void ce_sort_last_scan_res(struct wpa_supplicant *wpa_s)
{
	qsort(wpa_s->last_scan_res, wpa_s->last_scan_res_used,
		sizeof(*wpa_s->last_scan_res), ce_last_scan_res_compare);
}
