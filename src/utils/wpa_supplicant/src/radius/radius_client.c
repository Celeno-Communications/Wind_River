...
int g_keepalive_retry_init_radius_auth_client = 0;
int g_keepalive_retry_init_radius_acct_client = 0;

int g_retry_init_radius_auth_client = 0;
int g_retry_init_radius_acct_client = 0;
 ...
 */
#define RADIUS_CLIENT_FIRST_WAIT 1
...
enum radius_keepalive_state {
	RADIUS_KA_DOWN,
	RADIUS_KA_UP
};


struct radius_keepalive_data {

	int primary_auth_counter;
	int primary_acct_counter;
	int primary_auth_counter_back;
	int primary_acct_counter_back;
	enum radius_keepalive_state primary_auth_state;
	enum radius_keepalive_state primary_acct_state;
...
struct radius_client_data {
	...
	/**
	 * auth_keepalive_sock - IPv4 socket for authentication RADIUS 
	 * keepalive messages 
	 */
	int auth_keepalive_sock;

	/**
	 * auth_keepalive_sock - IPv6 socket for authentication RADIUS 
	 * keepalive messages 
	 */
	int auth_keepalive_sock6;
	...
	/**
	 * acct_keepalive_sock - IPv4 socket for accounting RADIUS 
	 * keepalive messages 
	 */
	int acct_keepalive_sock;

	/**
	 * acct_keepalive_sock - IPv6 socket for accounting RADIUS 
	 * keepalive messages 
	 */
	int acct_keepalive_sock6;
	...
	struct radius_keepalive_data keepalive_data;
};
				    ...
				    os_time_t now)
{
	...
	if (entry->msg_type == RADIUS_ACCT || entry->msg_type == RADIUS_ACCT_INTERIM)
		entry->next_wait = radius->conf->acct_timeout;
	else
		entry->next_wait = radius->conf->authentication_timeout;
...
static void radius_client_timer(void *eloop_ctx, void *timeout_ctx)
{
	...
	while (entry) {
		...
		if ((entry->attempts > conf->acct_retransmit_failure_th) && (entry->msg_type == RADIUS_ACCT || entry->msg_type == RADIUS_ACCT_INTERIM)) 
			acct_failover++; 
		else if ((entry->attempts > conf->retransmit_failure_th) && (entry->msg_type == RADIUS_AUTH))
			auth_failover++;
...
static void radius_client_auth_failover(struct radius_client_data *radius)
{
	...
	if (conf->enable_keepalive == 1) {
		radius_keepalive_change_auth_server(radius,1);
	} else {
		next = old + 1;
...
static void radius_client_acct_failover(struct radius_client_data *radius)
{
	...
	if (conf->enable_keepalive == 1) {
		radius_keepalive_change_acct_server(radius,1);
	} else {
		next = old + 1;
				   ...
				   size_t shared_secret_len, const u8 *addr)
{
	...
	if (msg_type == RADIUS_ACCT || msg_type == RADIUS_ACCT_INTERIM)
		entry->next_try = entry->first_try + radius->conf->acct_timeout;
	else
		entry->next_try = entry->first_try + radius->conf->authentication_timeout;
...
int radius_client_acct_keepalive_send(struct radius_client_data *radius, struct radius_msg *msg)
{
	...
	if (res >= 0)
	{
		if (radius->keepalive_data.primary_acct_state == RADIUS_KA_UP) 
			radius->keepalive_data.primary_acct_counter++;
		else // RADIUS_KA_DOWN
			radius->keepalive_data.primary_acct_counter_back++;
	}
...
int radius_client_auth_keepalive_send(struct radius_client_data *radius, struct radius_msg *msg)
{
	...
	if (res >= 0)
	{
		if (radius->keepalive_data.primary_auth_state == RADIUS_KA_UP) 
			radius->keepalive_data.primary_auth_counter++;
		else // RADIUS_KA_DOWN
			radius->keepalive_data.primary_auth_counter_back++;
	}
...
static void radius_client_auth_keepalive_receive(int sock, void *eloop_ctx, void *sock_ctx)
{
	struct radius_client_data *radius = eloop_ctx;
	int len;
	...
	if (len < 0) {
		...
		if (radius->keepalive_data.primary_auth_state == RADIUS_KA_UP)
			radius->keepalive_data.primary_auth_counter = 0;
		else
			radius->keepalive_data.primary_auth_counter_back = 0;
	}
}


static void radius_client_acct_keepalive_receive(int sock, void *eloop_ctx, void *sock_ctx)
{
	struct radius_client_data *radius = eloop_ctx;
	int len;
	unsigned char buf[3000];
	...
	if (len < 0) {
		...
		if (radius->keepalive_data.primary_acct_state == RADIUS_KA_UP)
			radius->keepalive_data.primary_acct_counter = 0;
		else
			radius->keepalive_data.primary_acct_counter_back = 0;
	}
}
...
static int
connect_radius_server(struct radius_client_data *radius,
		struct hostapd_radius_server *nserv,
		struct hostapd_radius_server *oserv,
		int sock, int sock6, int auth)
{
...
static void radius_client_retry_init_timer(void *eloop_ctx, void *timeout_ctx)
{
	struct radius_client_data *radius = eloop_ctx;
	struct hostapd_radius_servers *conf = radius->conf;

	if (conf->auth_server && g_retry_init_radius_auth_client) {
		int res = radius_client_init_auth(radius);

		if (!res)
			g_retry_init_radius_auth_client = 0;
		else if (ENETUNREACH != res)
			/* For every failure which isn't ENETUNREACH --> deinit. */
			radius_client_deinit(radius);
	}

	if (conf->acct_server && g_retry_init_radius_acct_client) {
		int res = radius_client_init_acct(radius);

		if (!res)
			g_retry_init_radius_acct_client = 0;
		else if (ENETUNREACH != res)
			/* For every failure which isn't ENETUNREACH --> deinit. */
			radius_client_deinit(radius);
	}
	...
	if (((conf->auth_server && g_retry_init_radius_auth_client)
	     || (conf->acct_server && g_retry_init_radius_acct_client))
	    && conf->radius_retry_init_interval) {
		eloop_register_timeout(conf->radius_retry_init_interval, 0,
				radius_client_retry_init_timer, radius,
				NULL);
	}
}
...
void radius_switch_auth_servers(struct radius_client_data *radius, int primary);

static int radius_client_init_keepalive_auth(struct radius_client_data *radius, int from_switch_servers);

void radius_keepalive_change_auth_server(struct radius_client_data *radius, int is_toggle)
{
	struct hostapd_radius_servers *conf = radius->conf;

	if (g_keepalive_retry_init_radius_auth_client) {
		if (radius_client_init_auth(radius)) {
			return;
		} else {
			int res = 0;
			g_keepalive_retry_init_radius_auth_client = 0;

			if (conf->auth_server && conf->enable_keepalive && (res = radius_client_init_keepalive_auth(radius,0))) {
				if (ENETUNREACH != res)
					radius_client_deinit(radius);
				return;
			}
	...
	if (conf->num_auth_servers > 1) {
		if (radius->keepalive_data.primary_auth_state == RADIUS_KA_UP) { 
			if (is_toggle || (radius->keepalive_data.primary_auth_counter > conf->keepalive_th)) {
				radius->keepalive_data.primary_auth_counter = 0;
				radius->keepalive_data.primary_auth_counter_back = 0;
				radius_switch_auth_servers(radius, 0);
				radius->keepalive_data.primary_auth_state = RADIUS_KA_DOWN;
			}
		} else {
			// RADIUS_KA_DOWN
			if (is_toggle || (radius->keepalive_data.primary_auth_counter_back > conf->keepalive_th)) {
				radius->keepalive_data.primary_auth_counter = 0;
				radius->keepalive_data.primary_auth_counter_back = 0;
				radius_switch_auth_servers(radius, 1);
				radius->keepalive_data.primary_auth_state = RADIUS_KA_UP;
			} else if (radius->keepalive_data.primary_auth_counter_back < radius->keepalive_data.primary_auth_counter) {
				radius->keepalive_data.primary_auth_counter = 0;
				radius->keepalive_data.primary_auth_counter_back = 0;
			}
		}
...
void radius_switch_acct_servers(struct radius_client_data *radius, int primary); 

static int radius_client_init_keepalive_acct(struct radius_client_data *radius, int from_switch_servers);

void radius_keepalive_change_acct_server(struct radius_client_data *radius, int is_toggle)
{
	struct hostapd_radius_servers *conf = radius->conf;

	if (g_keepalive_retry_init_radius_acct_client) {
		if (radius_client_init_acct(radius)) {
			return;
		} else {
			int res = 0;
			g_keepalive_retry_init_radius_acct_client = 0;

			if (conf->acct_server && conf->enable_keepalive && (res = radius_client_init_keepalive_acct(radius,0))) {
				if (ENETUNREACH != res)
					radius_client_deinit(radius);
				return;
			}
	...
	if (conf->num_acct_servers > 1) {
		if (radius->keepalive_data.primary_acct_state == RADIUS_KA_UP) {
			if (is_toggle || (radius->keepalive_data.primary_acct_counter > conf->acct_keepalive_th)) {
				radius->keepalive_data.primary_acct_counter = 0;
				radius->keepalive_data.primary_acct_counter_back = 0;
				radius_switch_acct_servers(radius, 0);
				radius->keepalive_data.primary_acct_state = RADIUS_KA_DOWN;
			}
		} else {
			// RADIUS_KA_DOWN
			if (is_toggle || (radius->keepalive_data.primary_acct_counter_back > conf->acct_keepalive_th)) {
				radius->keepalive_data.primary_acct_counter = 0;
				radius->keepalive_data.primary_acct_counter_back = 0;
				radius_switch_acct_servers(radius, 1);
				radius->keepalive_data.primary_acct_state = RADIUS_KA_UP;
			} else if (radius->keepalive_data.primary_acct_counter_back < radius->keepalive_data.primary_acct_counter) {
				radius->keepalive_data.primary_acct_counter = 0;
				radius->keepalive_data.primary_acct_counter_back = 0;
			}  
		}
...
void radius_switch_acct_servers(struct radius_client_data *radius, int primary)
{
	...
	if (primary) {
		// change to primary
		old = &(conf->acct_servers[conf->num_acct_servers - 1]);
		next = conf->acct_servers; 
	} else {
		// change to secondary
		old = conf->acct_servers;
		next = &(conf->acct_servers[conf->num_acct_servers - 1]);
	}

	conf->acct_server = next;
	radius_client_init_keepalive_acct(radius, 1);
...
void radius_switch_auth_servers(struct radius_client_data *radius, int primary)
{
	...
	if (primary) {
		// change to primary
		old = &(conf->auth_servers[conf->num_auth_servers - 1]);
		next = conf->auth_servers; 
	} else {
		// change to secondary
		old = conf->auth_servers;
		next = &(conf->auth_servers[conf->num_auth_servers - 1]);
	}

	conf->auth_server = next;
	radius_client_init_keepalive_auth(radius, 1);
...
static int radius_client_init_auth(struct radius_client_data *radius)
{
	...
	if (radius_change_server(radius, conf->auth_server, NULL,
				 radius->auth_serv_sock, radius->auth_serv_sock6, 1) == ENETUNREACH)
		return ENETUNREACH;
...
static int radius_client_init_keepalive_auth(struct radius_client_data *radius, int from_switch_servers)
{
	...
	radius->auth_keepalive_sock = socket(PF_INET, SOCK_DGRAM, 0);
	if (radius->auth_keepalive_sock < 0){
		perror("socket[PF_INET,SOCK_DGRAM]");
	}
#ifdef CONFIG_IPV6
	radius->auth_keepalive_sock6 = socket(PF_INET6, SOCK_DGRAM, 0);
	if (radius->auth_keepalive_sock6 < 0)
		perror("socket[PF_INET6,SOCK_DGRAM]");
#endif /* CONFIG_IPV6 */

	if (ENETUNREACH == connect_radius_server(radius, conf->auth_server, NULL,
				radius->auth_keepalive_sock, radius->auth_keepalive_sock6, 1)) {
		printf("Could not connect radius server via socket for authentication server\n");
		g_keepalive_retry_init_radius_auth_client = 1;
		return ENETUNREACH;
	}

	if (!from_switch_servers) {
		radius->keepalive_data.primary_auth_state = RADIUS_KA_UP;
		radius->keepalive_data.primary_auth_counter = 0;
		radius->keepalive_data.primary_auth_counter_back = 0;
	}
	...
	if (radius->auth_keepalive_sock >= 0 &&
			eloop_register_read_sock(radius->auth_keepalive_sock,
				radius_client_auth_keepalive_receive, radius,
				(void *) RADIUS_AUTH)) {
...
#ifdef CONFIG_IPV6
	if (radius->auth_keepalive_sock6 >= 0 &&
			eloop_register_read_sock(radius->auth_keepalive_sock6,
				radius_client_auth_keepalive_receive, radius,
				(void *) RADIUS_AUTH)) {
		printf("Could not register read socket for authentication "
				"server\n");
...
static int radius_client_init_keepalive_acct(struct radius_client_data *radius, int from_switch_servers)
{
	...
	radius->acct_keepalive_sock = socket(PF_INET, SOCK_DGRAM, 0);
	if (radius->acct_keepalive_sock < 0){
		perror("socket[PF_INET,SOCK_DGRAM]");
	}

#ifdef CONFIG_IPV6
	radius->acct_keepalive_sock6 = socket(PF_INET6, SOCK_DGRAM, 0);
	if (radius->acct_keepalive_sock6 < 0)
		perror("socket[PF_INET6,SOCK_DGRAM]");
#endif /* CONFIG_IPV6 */

	if (ENETUNREACH == connect_radius_server(radius, conf->acct_server, NULL,
				radius->acct_keepalive_sock, radius->acct_keepalive_sock6, 1)) {
		printf("Could not connect radius server via socket for accounting server\n");
		g_keepalive_retry_init_radius_acct_client = 1;
	}
	...
	if (radius->acct_keepalive_sock >= 0 &&
			eloop_register_read_sock(radius->acct_keepalive_sock,
				radius_client_acct_keepalive_receive, radius,
				(void *) RADIUS_ACCT)) {
		printf("Could not register read socket for authentication "
				"server\n");
		return -1;
...
#ifdef CONFIG_IPV6
	if (radius->acct_keepalive_sock6 >= 0 &&
			eloop_register_read_sock(radius->acct_keepalive_sock6,
				radius_client_acct_keepalive_receive, radius,
				(void *) RADIUS_ACCT)) {
...
static int radius_client_init_acct(struct radius_client_data *radius)
{
	...
	if (radius_change_server(radius, conf->acct_server, NULL,
				 radius->acct_serv_sock, radius->acct_serv_sock6, 0) == ENETUNREACH)
		return ENETUNREACH;
...
radius_client_init(void *ctx, struct hostapd_radius_servers *conf)
{
	...
	radius->auth_keepalive_sock = radius->acct_keepalive_sock = -1;
#ifdef CONFIG_IPV6
	radius->acct_keepalive_sock6 = radius->auth_keepalive_sock6 = -1;
#endif

	if (conf->auth_server && (res = radius_client_init_auth(radius))) {
		if (res != ENETUNREACH) {
			radius_client_deinit(radius);
			return NULL;
		}
	...
	if (conf->auth_server) {
		int res = 0;
		if ((res = radius_client_init_keepalive_auth(radius, 0))) {
			if(res != ENETUNREACH) {
				radius_client_deinit(radius);
				return NULL;
			}

			if (conf->enable_keepalive)
				g_keepalive_retry_init_radius_auth_client = 1;
			else
				g_retry_init_radius_auth_client = 1;
		}
	}

	if (conf->acct_server && (res = radius_client_init_acct(radius))) {
		if (res != ENETUNREACH) {
			radius_client_deinit(radius);
			return NULL;
		}
	...
	if (conf->acct_server) {
		int res = 0;
		if ((res = radius_client_init_keepalive_acct(radius, 0))) {
			if (res != ENETUNREACH) {
				radius_client_deinit(radius);
				return NULL;
			}
			...
			if (conf->enable_keepalive)
				...
				g_retry_init_radius_acct_client = 1;
		}
	}

	if (conf->radius_retry_init_interval)
		eloop_register_timeout(conf->radius_retry_init_interval, 0,
				       radius_client_retry_init_timer, radius,
				       NULL);
...
void radius_client_deinit(struct radius_client_data *radius)
{
	...
	if (radius->auth_keepalive_sock >= 0)
		eloop_unregister_read_sock(radius->auth_keepalive_sock);
	if (radius->acct_keepalive_sock >= 0)
		eloop_unregister_read_sock(radius->acct_keepalive_sock);
#ifdef CONFIG_IPV6		
	if (radius->auth_keepalive_sock6 >= 0)
		eloop_unregister_read_sock(radius->auth_keepalive_sock6);
	if (radius->acct_keepalive_sock6 >= 0)
		eloop_unregister_read_sock(radius->acct_keepalive_sock6);
#endif

	eloop_cancel_timeout(radius_client_retry_init_timer, radius, NULL);
