			    ...
			    struct he_capabilities *he_cap)
{
		...
		if (!center_segment0) {
			...
			if (center_segment1) {
				if (center_idx_to_bw_6ghz(center_segment1) < 2 ||
				    bw != 2) {
					wpa_printf(MSG_ERROR,
						   "6 GHz 80+80/160 MHz configuration doesn't use valid 80 MHz channels");
					return -1;
				}
	...
	case CHANWIDTH_160MHZ:
		if (!(vht_caps & (VHT_CAP_SUPP_CHAN_WIDTH_160MHZ |
		    ...
		    !(vht_caps & VHT_CAP_EXT_NSS_BW_MASK)) {
			wpa_printf(MSG_ERROR,
	...
	case CHANWIDTH_160MHZ:
		...
		if (center_segment1)
			/* center_segment1 contains the center freq of 160MHz
			   so use it to make the calculations below */
