				      ...
				      struct radius_msg *msg)
{
	...
	os_snprintf(buf, sizeof(buf),
		    hapd->conf->radius_addr_format_low ? RADIUS_802_1X_ADDR_FORMAT_LOW_REG : RADIUS_802_1X_ADDR_FORMAT,
		    MAC2STR(sta->addr));
	buf[sizeof(buf) - 1] = '\0';
			   ...
			   struct radius_msg *msg)
{
	...
	if (!hostapd_config_get_radius_attr(req_attr,
	    ...
	    !hapd->conf->radius_use_nas_ipv6_addr &&
	...
	if (hapd->conf->radius_sta_id_enable) {
		if (hapd->conf->radius_addr_format_low) {
			len = os_snprintf(buf, sizeof(buf), RADIUS_802_1X_ADDR_FORMAT_LOW_REG ":%s",
					  MAC2STR(hapd->conf->radius_cm_mac),
					  wpa_ssid_txt(hapd->conf->ssid.ssid,
						       hapd->conf->ssid.ssid_len));
		} else {
			len = os_snprintf(buf, sizeof(buf), RADIUS_802_1X_ADDR_FORMAT ":%s",
					  MAC2STR(hapd->conf->radius_cm_mac),
					  wpa_ssid_txt(hapd->conf->ssid.ssid,
						     hapd->conf->ssid.ssid_len));
		}
	} else if (hapd->conf->radius_addr_format_low) {
		len = os_snprintf(buf, sizeof(buf), RADIUS_802_1X_ADDR_FORMAT_LOW_REG ":%s",
	...
	for (attr = req_attr; attr; attr = attr->next) {
		if (attr->type == RADIUS_ATTR_NAS_IP_ADDRESS &&
				hapd->conf->radius_use_nas_ipv6_addr)
			continue;
		if (attr->type == RADIUS_ATTR_NAS_IPV6_ADDRESS &&
				!hapd->conf->radius_use_nas_ipv6_addr)
			continue;
...
#define celeno_cb_fn "/usr/sbin/rpc_wlan_reverse_client 192.168.254.253 FwdId"

struct radius_msg_data {
	u8           msg_type;
	u8           msg_len;
	uint32_t     vendor_id;
	u8           msg_vendor_type;
				...
				size_t shared_secret_len)
{
	...
	char cmd [124];
	size_t i;
	struct radius_msg_data *rmd;

	if (hapd->conf->radius_fwd_id_enable && msg) {
		for (i = 0; i < msg->attr_used; i++) {
			attr = radius_get_attr_hdr(msg, i);
			if (attr->type == RADIUS_ATTR_VENDOR_SPECIFIC) {
				rmd = (struct radius_msg_data*) attr;
				if ((rmd->msg_len >= sizeof(struct radius_msg_data)) &&
						(rmd->vendor_id == 9134) &&
						(rmd->msg_vendor_type == 11)) {
					memset(cmd, 0, sizeof(cmd));
					fwd_id = rmd->payload;
					ifname = strtol(hapd->conf->iface + strlen("wdev0ap"), NULL, 10);
					os_snprintf(cmd, sizeof(cmd), "%s,%d,%d,%u", celeno_cb_fn, 1, ifname, fwd_id);
					(void)!system(cmd);
				}
			}
		}
			...
			void *data)
{
	...
	case RADIUS_CODE_ACCESS_REJECT:
		...
		radius_try_holdoff_sta(hapd, sta->addr);
...
#ifdef HOSTAPD
static struct radius_msg * auth_server_status_msg(struct hostapd_data *hapd)					  
{
	struct radius_msg *msg;
	msg = radius_msg_new(RADIUS_CODE_STATUS_SERVER,
...
void auth_keepalive_send(struct hostapd_data *hapd)
{
	struct radius_msg *msg;
	...
	msg = auth_server_status_msg(hapd);

	if (msg &&radius_client_auth_keepalive_send(hapd->radius, msg) < 0)
		radius_msg_free(msg);
}

static void auth_keepalive(void *eloop_ctx, void *timeout_ctx)
{
	struct hostapd_data *hapd = eloop_ctx;

	radius_keepalive_change_auth_server(hapd->radius,0);
	auth_keepalive_send(hapd);    
	eloop_register_timeout(hapd->conf->radius->keepalive_interval, 0, auth_keepalive,  hapd, NULL);
}
#endif /* HOSTAPD */
...
#ifdef HOSTAPD
	if (hapd->conf->radius->enable_keepalive)
		eloop_register_timeout(hapd->conf->radius->keepalive_interval, 0, auth_keepalive, hapd, NULL);
#endif /* HOSTAPD */
...
#ifdef HOSTAPD
	if (hapd->conf->radius->enable_keepalive)  
		eloop_cancel_timeout(auth_keepalive, hapd, NULL);
#endif /* HOSTAPD */
