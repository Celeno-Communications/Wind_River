...
#define ACCT_DEFAULT_UPDATE_INTERVAL 60
...
static void accounting_interim_update(void *eloop_ctx, void *timeout_ctx)
{
	...
	if (sta->acct_interim_interval ||
	    hapd->conf->radius->acct_server->acct_interim_interval) {
		accounting_sta_interim(hapd, sta);

		if (hapd->conf->radius->acct_server->acct_interim_interval)
			interval = hapd->conf->radius->acct_server->acct_interim_interval;
		else
...
static struct radius_msg * acct_server_status_msg(struct hostapd_data *hapd)					  
{
	struct radius_msg *msg;
...
void accounting_keepalive_send(struct hostapd_data *hapd)
{
	struct radius_msg *msg;
	...
	msg = acct_server_status_msg(hapd);

	if (msg &&radius_client_acct_keepalive_send(hapd->radius, msg) < 0)
		radius_msg_free(msg);
}

static void accounting_keepalive(void *eloop_ctx, void *timeout_ctx)
{
	struct hostapd_data *hapd = eloop_ctx;

	radius_keepalive_change_acct_server(hapd->radius,0);
	accounting_keepalive_send(hapd);
	eloop_register_timeout(hapd->conf->radius->acct_keepalive_interval, 0, accounting_keepalive, hapd, NULL);
}
...
void accounting_sta_start(struct hostapd_data *hapd, struct sta_info *sta)
{
	...
	if (hapd->conf->radius->acct_server->acct_interim_interval)
		interval = hapd->conf->radius->acct_server->acct_interim_interval;
	else if (sta->acct_interim_interval)
		interval = sta->acct_interim_interval;
	else
		interval = ACCT_DEFAULT_UPDATE_INTERVAL;
	eloop_register_timeout(interval, 0, accounting_interim_update,
...
{
	if (hapd->conf->radius->enable_keepalive)
		eloop_cancel_timeout(accounting_keepalive, hapd, NULL);
