...
extern void send_deauth(struct hostapd_data *hapd, const u8 *addr, u16 reason_code);
...
skip_poll:
	...
	case STA_DISASSOC_FROM_CLI:
		...
		send_deauth(hapd, sta->addr, WLAN_REASON_CLASS3_FRAME_FROM_NONASSOC_STA);
		break;
	case STA_DEAUTH:
		hostapd_logger(hapd, sta->addr, HOSTAPD_MODULE_IEEE80211,
				HOSTAPD_LEVEL_INFO, "deauthenticated due to "
		...
		send_deauth(hapd, sta->addr, WLAN_REASON_CLASS2_FRAME_FROM_NONAUTH_STA);
		ap_free_sta(hapd, sta);
		break;
	case STA_REMOVE:
		hostapd_logger(hapd, sta->addr, HOSTAPD_MODULE_IEEE80211,
...
static struct mac_entry *find_entry(struct mac_entry *list, const u8 *addr)
{
	int i;

	for (i = 0; i < HOLDOFF_LIST_SIZE; i++) {
		if (!list[i].nonempty)
			continue;
		if (!os_memcmp(list[i].addr, addr, ETH_ALEN))
			return &list[i];
	}
...
static struct mac_entry *find_empty_entry(struct mac_entry *list)
{
	int i;
...
static void del_entry(struct mac_entry *entry)
{
	if (entry->state & STA_ST_THRESHOLD)
		wpa_printf(MSG_DEBUG, "Remove Sta: " MACSTR " from Threshold list", MAC2STR(entry->addr));

	if (entry->state & STA_ST_HOLDOFF)
		wpa_printf(MSG_DEBUG, "Remove Sta: " MACSTR " from Hold-Off list", MAC2STR(entry->addr));

	os_memset(entry, 0, sizeof(struct mac_entry));
}

void clean_entry_timer(void *entry, void *user_ctx)
{
	if (entry)
		del_entry((struct mac_entry *)entry);
}

static struct mac_entry *add_entry(struct mac_entry *list, const u8 *addr)
{
	struct mac_entry *entry = find_empty_entry(list);
...
void radius_try_holdoff_sta(struct hostapd_data *hapd, const u8 *addr)
{
	struct mac_entry *entry;
	int holdoff_timeout = hapd->conf->radius->auth_holdoff_timeout;
	int holdoff_threshold = hapd->conf->radius->auth_holdoff_threshold;
	int holdoff_retries = hapd->conf->radius->auth_holdoff_retries;

	if (!holdoff_retries)
		return;

	entry = find_entry(hapd->radius_holdoff_list, addr);
	if (!entry)
		entry = add_entry(hapd->radius_holdoff_list, addr);

	if (!entry) {
		wpa_printf(MSG_DEBUG,"Can't add " MACSTR " to Threshold list", MAC2STR(addr));
		return;
	}
	...
	switch (entry->state) {
		...
		case STA_ST_THRESHOLD:
			entry->count++;
			eloop_cancel_timeout(clean_entry_timer, entry, NULL);
			if (entry->count > holdoff_retries) {
				entry->state = STA_ST_HOLDOFF;
				eloop_register_timeout(holdoff_timeout, 0,
						clean_entry_timer, entry, NULL);
			}

			eloop_register_timeout(holdoff_threshold, 0,
					clean_entry_timer, entry, NULL);

			break;

		default:
			entry->state = STA_ST_THRESHOLD;
			entry->count++;
			eloop_register_timeout(holdoff_threshold, 0,
					       clean_entry_timer, entry, NULL);
	}
...
int radius_check_holdoff_sta(struct hostapd_data *hapd, const u8 *addr)
{
	struct mac_entry *entry;

	entry = find_entry(hapd->radius_holdoff_list, addr);
	if (entry && (entry->state & STA_ST_HOLDOFF))
		return STA_REJECT;
