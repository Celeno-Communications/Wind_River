...
u8 * hostapd_eid_ht_operation(struct hostapd_data *hapd, u8 *eid)
{
	...
	*pos++ = sizeof(*oper);
	...
	/* Set center freq seg2 in HT operation mode only if needed */
	if (hostapd_get_center_freq_seg1_location(hapd->iconf) == HOSTAPD_CENTER_FREQ_HT_OPER)
		oper->operation_mode |=
		((hapd->iconf->ht_oper_centr_freq_seg2_idx <<
		  HT_OPER_OP_MODE_CHAN_CENTER_FREQ_SEG2_SHIFT) &
		 HT_OPER_OP_MODE_CHAN_CENTER_FREQ_SEG2_MASK);
