				 ...
				 struct wpabuf *respData)
{
	...
	if (sm->identity == NULL) {
		...
		extern bool wps_is_win7_not_pbc(struct wps_registrar *reg);

		/* Ignore External Registrar requests to avoid WIN7 problem.
		   Run it in case of configured state of WPS and not active PBC */
		if (sm->wps &&
		    sm->wps->wps_state == WPS_STATE_CONFIGURED &&
		    sm->wps->registrar &&
		    wps_is_win7_not_pbc(sm->wps->registrar)) {
			char *identity_copy = os_malloc(len + 1);
			if (!identity_copy) {
				data->state = FAILURE;
				return;
			}
			identity_copy[len] = '\0';
			os_memcpy(identity_copy, pos, len);
			if (strstr(identity_copy, "WFA-SimpleConfig-Registrar-1-0")) {
				wpa_printf(MSG_DEBUG, "EAP-Identity: Registrar detected");
				os_free(identity_copy);
				data->state = FAILURE;
				return;
			}
			os_free(identity_copy);
