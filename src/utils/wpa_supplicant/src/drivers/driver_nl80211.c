				   ...
				   const struct hostapd_freq_params *freq)
{
	...
	if (freq->vht_enabled || (freq->he_enabled && !is_24ghz)) {
		...
		u32 freq1 = freq->center_freq1, freq2 = freq->center_freq2;
		...
		case 80:
			if (freq2) {
				if (freq2 - freq1 > 80) {
					...
					/* Convert to the old style since this is
					 * the format that nl80211 expects
					 */
		...
		wpa_printf(MSG_DEBUG, "  * center_freq1=%d", freq1);
		wpa_printf(MSG_DEBUG, "  * center_freq2=%d", freq2);
