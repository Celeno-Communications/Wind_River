				    ...
				    const char *capab)
{
	...
	if (os_strstr(capab, "[CAP-EXT-NSS-BW_1]"))
		conf->vht_capab |= VHT_CAP_EXT_NSS_BW_1;
	if (os_strstr(capab, "[CAP-EXT-NSS-BW_2]"))
		conf->vht_capab |= VHT_CAP_EXT_NSS_BW_2;
	if (os_strstr(capab, "[CAP-EXT-NSS-BW_3]"))
			       ...
			       const char *buf, char *pos, int line)
{
	if (os_strcmp(buf, "interface") == 0) {
	...
	} else if (os_strcmp(buf, "enable_keepalive") == 0) { 
		bss->radius->enable_keepalive = atoi(pos);
	} else if (os_strcmp(buf, "keepalive_interval") == 0) {
		bss->radius->keepalive_interval = atoi(pos);
	}else if (os_strcmp(buf, "keepalive_threshold") == 0) {
		bss->radius->keepalive_th = atoi(pos);
	} else if (os_strcmp(buf, "acct_keepalive_interval") == 0) {
		bss->radius->acct_keepalive_interval= atoi(pos);
	} else if (os_strcmp(buf, "acct_keepalive_threshold") == 0) {
		bss->radius->acct_keepalive_th = atoi(pos);
	} else if (os_strcmp(buf, "retransmit_failure_threshold") == 0) {
		bss->radius->retransmit_failure_th = atoi(pos);
	} else if (os_strcmp(buf, "acct_retransmit_failure_threshold") == 0) {
		bss->radius->acct_retransmit_failure_th= atoi(pos);
	} else if (os_strcmp(buf, "authentication_timeout") == 0) {
		bss->radius->authentication_timeout= atoi(pos);
	} else if (os_strcmp(buf, "auth_holdoff_retries") == 0) {
		bss->radius->auth_holdoff_retries = atoi(pos);
	} else if (os_strcmp(buf, "auth_holdoff_timeout") == 0) {
		bss->radius->auth_holdoff_timeout = atoi(pos);
	} else if (os_strcmp(buf, "auth_holdoff_threshold") == 0) {
		bss->radius->auth_holdoff_threshold = atoi(pos);
	} else if (os_strcmp(buf, "acct_timeout") == 0) {
		bss->radius->acct_timeout = atoi(pos);
	} else if (bss->radius->acct_server &&
	...
	} else if (os_strcmp(buf, "radius_retry_init_interval") == 0) {
		bss->radius->radius_retry_init_interval = atoi(pos);
	} else if (os_strcmp(buf, "radius_use_nas_ipv6_addr") == 0) {
		bss->radius_use_nas_ipv6_addr = atoi(pos);
	} else if (bss->radius->acct_server && os_strcmp(buf, "radius_acct_interim_interval") == 0) {
		bss->radius->acct_server->acct_interim_interval = atoi(pos);
	...
	} else if (os_strcmp(buf, "ha_is_low_reg_mac_fmt") == 0) {
		bss->radius_addr_format_low = !!(int) strtol(pos, NULL, 10);
	} else if (os_strcmp(buf, "forward_id_enable") == 0) {
		bss->radius_fwd_id_enable = strtol(pos, NULL, 10);
	} else if (os_strcmp(buf, "radius_sta_id_enable") == 0) {
		bss->radius_sta_id_enable = strtol(pos, NULL, 10);
	} else if (os_strcmp(buf, "CM-MAC") == 0) {
		int len = sscanf(pos, "%02hhx-%02hhx-%02hhx-%02hhx-%02hhx-%02hhx",
				 &bss->radius_cm_mac[0], &bss->radius_cm_mac[1], &bss->radius_cm_mac[2],
				 &bss->radius_cm_mac[3], &bss->radius_cm_mac[4], &bss->radius_cm_mac[5]);
		if (len != ETH_ALEN) {
			wpa_printf(MSG_ERROR, "Incorrect nvram param CM-MAC. Use format ff-ff-ff-ff-ff-ff");
			memset(bss->radius_cm_mac, 0, ETH_ALEN);
		}
		...
		if (hostapd_parse_intlist(&bss->disabled_ht_mcs, pos)) {
			wpa_printf(MSG_ERROR, "Line %d: invalid dis ht mcs",
				   line);
			return 1;
		}
	...
	} else if (os_strcmp(buf, "ht_oper_centr_freq_seg2_idx") == 0) {
		conf->ht_oper_centr_freq_seg2_idx = atoi(pos);
#ifdef CONFIG_IEEE80211AC
...
#define PARSE_TEST_PROBABILITY(_val)				\
	...
	} else if (os_strcmp(buf, "ha_wps_win7_support") == 0) {
		conf->ha_wps_win7_support = strtol(pos, NULL, 10);
#ifdef CONFIG_PASN
#ifdef CONFIG_TESTING_OPTIONS
