...
int wpa_debug_change_file(int file_to_close, int file_to_open);
int wpa_debug_open_file(int file_to_open,char* mode);
...
static FILE* files[NUM_OF_FILES];
static char* path [NUM_OF_FILES];


void vfprintf_selected_file(const char *format, va_list arg)
{
	vfprintf(files[selected_file], format, arg);
	if (ftell(files[selected_file]) > FILE_MAX_SIZE) {
		int new_file = (selected_file + 1) % NUM_OF_FILES;
		wpa_debug_change_file(selected_file, new_file);
		selected_file = new_file;
	}
}


void fprintf_selected_file(const char* format, ...)
{
	va_list ap;
	va_start(ap,format);
	vfprintf_selected_file(format,ap);
	va_end(ap);
}
...
void wpa_debug_print_timestamp(void)
{
	...
	if (files[selected_file])
		fprintf_selected_file("%ld.%06u: ",(long) tv.sec,(unsigned int) tv.usec);
#endif /* CONFIG_DEBUG_FILE */
...
void wpa_printf(int level, const char *fmt, ...)
{
	...
	if (level >= wpa_debug_level) {
		...
		if (files[selected_file]) {
			va_start(ap, fmt);
			vfprintf_selected_file(fmt, ap);
			fprintf_selected_file("\n");
			va_end(ap);
		}
#endif /* CONFIG_DEBUG_FILE */
...
#ifdef CONFIG_DEBUG_FILE
	if (files[selected_file]) {
		fprintf_selected_file("%s - hexdump(len=%lu):", title, (unsigned long) len);
		if (buf == NULL) {
			fprintf_selected_file(" [NULL]");
		} else if (show) {
			for (i = 0; i < len; i++)
				fprintf_selected_file(" %02x", buf[i]);
		} else {
			fprintf_selected_file(" [REMOVED]");
		}
		fprintf_selected_file("\n");
	}
#endif /* CONFIG_DEBUG_FILE */
			       ...
			       size_t len, int show)
{
	...
	if (files[selected_file]) {
		if (!show) {
			fprintf_selected_file("%s - hexdump_ascii(len=%lu): [REMOVED]\n",title, (unsigned long) len);
			goto file_done;
		}
		if (buf == NULL) {
			fprintf_selected_file("%s - hexdump_ascii(len=%lu): [NULL]\n",
				title, (unsigned long) len);
			goto file_done;
		}
		fprintf_selected_file("%s - hexdump_ascii(len=%lu):\n",
			title, (unsigned long) len);
		while (len) {
			llen = len > line_len ? line_len : len;
			fprintf_selected_file("    ");
			for (i = 0; i < llen; i++)
				fprintf_selected_file(" %02x", pos[i]);
			for (i = llen; i < line_len; i++)
				fprintf_selected_file("   ");
			fprintf_selected_file("   ");
			for (i = 0; i < llen; i++) {
				if (isprint(pos[i]))
					fprintf_selected_file("%c", pos[i]);
				else
					fprintf_selected_file("_");
			}
			for (i = llen; i < line_len; i++)
				fprintf_selected_file(" ");
			fprintf_selected_file("\n");
...
void wpa_debug_close_file(int file_to_close)
{
#ifdef CONFIG_DEBUG_FILE
	if (!files[file_to_close])
		return;		

	fclose(files[file_to_close]);
	files[file_to_close] = NULL;
	os_free(path[file_to_close]);
	path[file_to_close] = NULL;
#endif /* CONFIG_DEBUG_FILE */
}
...
int wpa_debug_reopen_file(void)
{
	...
	tmp = os_strdup(path[selected_file]);
	if (!tmp)
		return -1;

	wpa_debug_close_file(selected_file);
	rv = wpa_debug_open_file(selected_file, "a");
	path[selected_file] = os_strdup(tmp);
...
int wpa_debug_init_files(const char *file_path)
{
#ifdef CONFIG_DEBUG_FILE
	int i;
	...
	for (i=0;i<NUM_OF_FILES;i++) {
		...
		path[i] = os_strdup(file_path);
		sprintf(file_number,"%d",i);	        
		strcat(path[i],file_number);
	}

	files[0] = fopen(path[0], "w");
	if (files[0] == NULL) {
		wpa_printf(MSG_ERROR, "wpa_debug_init_files: Failed to open "
				"first output file, using standard output");
		return -1;
	}
#ifndef _WIN32
	setvbuf(files[0], NULL, _IOLBF, 0);
#endif /* _WIN32 */
#endif /* CONFIG_DEBUG_FILE */
	return 0;
}
...
int wpa_debug_change_file(int file_to_close, int file_to_open)
{
	...
	if(path[file_to_open]) {
		if(files[file_to_close] != NULL) {
			fclose(files[file_to_close]);
			files[file_to_close] = NULL;
		}
		rv = wpa_debug_open_file(file_to_open,"w");
	} else {
		wpa_printf(MSG_ERROR, "path is not set cannot open change file.\n");
		rv = -1;
	}
	return rv;
#else /* CONFIG_DEBUG_FILE */
...
int wpa_debug_open_file(int file_to_open,char* mode)
{
	...
	files[file_to_open] = fopen(path[file_to_open], mode);
	if (files[file_to_open] == NULL) {
		wpa_printf(MSG_ERROR, "wpa_debug_open_file: Failed to open "
				"first output file, using standard output");
		return -1;
	}
#ifndef _WIN32
	setvbuf(files[file_to_open], NULL, _IOLBF, 0);
#endif /* _WIN32 */
#endif /* CONFIG_DEBUG_FILE */
	return 0;
}
...
void wpa_debug_close_files(void)
{
#ifdef CONFIG_DEBUG_FILE
	int i; 
	for (i = 0; i < NUM_OF_FILES; i ++) {
