...
u8 hostapd_get_actual_chwidth(struct hostapd_config *conf)
{
	u8 ret = CHANWIDTH_USE_HT;
	u8 seg1;

	if (conf->vht_oper_chwidth != CHANWIDTH_80MHZ)
		return conf->vht_oper_chwidth;

	if (conf->vht_oper_centr_freq_seg1_idx != 0)
		seg1 = conf->vht_oper_centr_freq_seg1_idx;
	else
		seg1 = conf->ht_oper_centr_freq_seg2_idx;
...
u8 hostapd_get_center_freq_seg1_location(struct hostapd_config *conf)
{
	u32 own_cap = conf->vht_capab;
	u8 ext_nss_bw_supp = (own_cap & VHT_CAP_EXT_NSS_BW_MASK) >> VHT_CAP_EXT_NSS_BW_SHIFT;
	u8 supp_chwidth = (own_cap & VHT_CAP_SUPP_CHAN_WIDTH_MASK) >> VHT_CAP_SUPP_CHAN_WIDTH_MASK_SHIFT;
	u8 res = HOSTAPD_CENTER_FREQ_NONE;
	u8 actual_chanwidth = hostapd_get_actual_chwidth(conf);

	/* The bandwidth is less than 80+80/160MHz */
	if (actual_chanwidth < CHANWIDTH_160MHZ)
		return HOSTAPD_CENTER_FREQ_NONE;

	switch (supp_chwidth) {
	case 0:
		if ((ext_nss_bw_supp > 1) ||
		    ((ext_nss_bw_supp == 1) &&
		     (actual_chanwidth == CHANWIDTH_160MHZ)))
			res = HOSTAPD_CENTER_FREQ_HT_OPER; /* Use CCFS2 */
		break;
	case 1:
		if ((ext_nss_bw_supp > 2) ||
		    (actual_chanwidth == CHANWIDTH_160MHZ))
			res = HOSTAPD_CENTER_FREQ_VHT_OPER; /* Use CCFS1 */
		else if (ext_nss_bw_supp > 0)
			res = HOSTAPD_CENTER_FREQ_HT_OPER; /* Use CCFS2 */
		break;
	case 2:
		res = HOSTAPD_CENTER_FREQ_VHT_OPER;
		if ((ext_nss_bw_supp > 0) && (ext_nss_bw_supp < 3))
			wpa_printf(MSG_ERROR, "Invalid value %u for ext_nss_bw_supp and supp_chwidth 2", ext_nss_bw_supp);
		break;
	default:
		break;
	}
