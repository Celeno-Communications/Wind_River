...
#endif /* CONFIG_IEEE80211AX */
	if (radius_check_holdoff_sta(hapd, sta->addr))
		return -1;
			   ...
			   int omit_rsnxe)
{
	...
	p = hostapd_eid_rm_enabled_capab(hapd, p, (u8*)reply + sizeof(buf) - p);
...
u8 * hostapd_eid_wb_chsw_wrapper(struct hostapd_data *hapd, u8 *eid)
{
	...
	case 80:
		...
		if (!hapd->cs_freq_params.center_freq2 ||
		    (abs(hapd->cs_freq_params.center_freq2 -
			 hapd->cs_freq_params.center_freq1) == 40))
			bw = 1;
		else
			bw = 3;
		break;
