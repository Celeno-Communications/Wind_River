...
static int hapd_wps_cred_cb(struct hostapd_data *hapd, void *ctx)
{
	...
	sscanf(hapd->conf->iface, "wlan%u_0", &if_index);
	snprintf(buf, sizeof(buf), "nvram_set %d ha_ssid1 %s && nvram_set %d ha_wpa_passphrase1 %s", 
		if_index, cred->ssid, if_index, cred->key);
	(void)!system(buf);
...
static int clone_uuid_to_second_band(struct hostapd_data *hapd, u8* uuid)
{
	char uuid_buf[50];
	char buf[1024];

	if (uuid_bin2str(uuid, uuid_buf, sizeof(uuid_buf)) < 0) {
		wpa_printf(MSG_ERROR, "%s: failed to convert uuid from hex to string \n", __func__);
		return -1;
	} else {
		snprintf(buf, sizeof(buf), "nvram_set common uuid1 %s", uuid_buf);
		(void)!system(buf);
	}
		     ...
		     struct hostapd_bss_config *conf)
{
	...
	if (clone_uuid_to_second_band(hapd, wps->uuid) < 0)
		wpa_printf(MSG_ERROR, "Failed to clone UUID to second_band");
	...
	cfg.ha_wps_win7_support = hapd->iconf->ha_wps_win7_support;
