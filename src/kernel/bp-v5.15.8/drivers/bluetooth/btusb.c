...
#if IS_ENABLED(CPTCFG_BT_CL)
	/* Celeno-specific devices */
	{ USB_VENDOR_AND_INTERFACE_INFO(0x33de, 0xe0, 0x01, 0x01),
	  .driver_info = BTUSB_IFNUM_2},
#endif
...
static struct urb *_alloc_ctrl_urb(struct hci_dev *hdev, struct sk_buff *skb)
{
	struct btusb_data *data = hci_get_drvdata(hdev);
	struct usb_interface *intf = data->intf;
	struct usb_ctrlrequest *dr;
	struct urb *urb;
	unsigned int pipe;
	...
	dr->bRequestType = data->cmdreq_type;
	dr->bRequest     = data->cmdreq;
	dr->wIndex       = intf->cur_altsetting->desc.bInterfaceNumber;
	dr->wValue       = 0;
	dr->wLength      = __cpu_to_le16(skb->len);
...
#if IS_ENABLED(CPTCFG_BT_CL)
struct cl_chip *btusb_hci_to_chip_device(struct hci_dev *hdev)
{
	struct btusb_data *data = hci_get_drvdata(hdev);
	return (struct cl_chip *)dev_get_drvdata(&data->udev->dev);
}

static int btusb_send_frame_celeno(struct hci_dev *hdev, struct sk_buff *skb)
{
	struct urb *urb;
...
#if IS_ENABLED(CPTCFG_BT_CL)
	if (id->driver_info & BTUSB_IFNUM_2) {
		hdev->send = btusb_send_frame_celeno;
		btcl_init_celeno(hdev, btusb_hci_to_chip_device);
	}
#endif
