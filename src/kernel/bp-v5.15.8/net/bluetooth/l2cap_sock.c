...
static int backport_l2cap_sock_getname(struct socket *sock, struct sockaddr *addr,
			      int *len, int peer)
{
	int ret;
	ret = l2cap_sock_getname(sock, addr, peer);
	if (ret >= 0) {
		*len = ret;
		ret = 0;
	}
...
#if LINUX_VERSION_IS_LESS(4,17,0)
	.getname	= backport_l2cap_sock_getname,
#else
