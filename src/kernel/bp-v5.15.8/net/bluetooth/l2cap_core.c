...
static void l2cap_chan_le_send_credits(struct l2cap_chan *chan)
{
	...
	/* Sender should always receive credits more than it could fit into
	 * one connection interval. Otherwise it can cause interruptions while
	 * waiting for more credits.
	 */
	/* 1. Calculate rx credits for 1 connection interval */
	le_max_credits = (chan->imtu / chan->mps) + 1;

	/* 2. Increase this vlaue to 3 connection intervals. */
	le_max_credits *= L2CAP_LE_MIN_CON_INTERVAL_NUM;

	/* 3. For small values, allocate at least L2CAP_LE_MIN_CREDITS */
	if (le_max_credits < L2CAP_LE_MIN_CREDITS)
		le_max_credits = L2CAP_LE_MIN_CREDITS;
